...
class NoticeController extends Controller {
    ...
    public function actionIndex() {
        if (Yii::$app->user->can('seting-app')) {
    ...
    public function actionCreate() {
        if (Yii::$app->user->can('seting-app')) {
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                $model->notice_date_end = date('Y-m-d', strtotime($_POST['Notice']['notice_date_end']));
                $model->created_by = Yii::$app->getid->getId();
                $model->created_at = new \yii\db\Expression('NOW()');
                $model->notice_file_path = UploadedFile::getInstance($model, 'notice_file_path');
                if ($model->notice_file_path) {
                    $model->notice_file_path->saveAs(Yii::getAlias('@webroot') . '/notice/' . $model->notice_file_path = 'Notice_' . date("d-m-Y_His") . '.' . $model->notice_file_path->extension);
                } else
                    $model->notice_file_path = NULL;

                if ($model->save(false)){
                    $modelevent = new \backend\modules\dashboard\models\Events([
                        'event_title' =>$model->notice_title, 
                        'event_detail' => $model->notice_description,
                        'event_start_date' => $model->notice_date,
                        'event_end_date' => $model->notice_date_end,
                        'event_url' => $model->notice_file_path,
                        'event_type' => 3,
                    ]);
                $modelevent->save();
    ...
    public function actionUpdate($id) {
        if (Yii::$app->user->can('seting-app')) {
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($model->notice_file_path) {
                    $Dfile = Yii::getAlias('@webroot') . '/notice/' . $old_model->notice_file_path;
    ...
    public function actionNoticeFile($nid) {
        $path = Yii::getAlias('@webroot') . '/notice/';
